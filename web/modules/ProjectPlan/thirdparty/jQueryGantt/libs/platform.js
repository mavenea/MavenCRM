...
//----------------------------------positioning-----------------------------------------------
jQuery.fn.bringToFront=function(selector){
  var zi=10;
  var elements = selector ? jQuery(selector) : jQuery("*");
  ...
  return jQuery(this).css('zIndex',zi+=10);
};
...
function nearBestPosition(whereId, theObjId, centerOnEl) {
  ...
  if (typeof whereId != "object"){ el = jQuery("#"+whereId); }
  if (typeof theObjId != "object"){target = jQuery("#"+theObjId);}
  ...
  if (el) {
    ...
    var left = trueX;
    var top = trueY;
    var barHeight = (jQuery.browser.msie) ? 45 : 35;
    var barWidth = (jQuery.browser.msie) ? 20 : 0;
    ...
    if (target.offset().left >= (jQuery(window).width() - target.outerWidth())) {
      left = (jQuery(window).width() - target.outerWidth() - barWidth )+ "px";
      target.css("left", left);
    }
    ...
    if ((target.offset().top  + target.outerHeight() >= ((jQuery(window).height() - barHeight))) && (target.outerHeight() < jQuery(window).height())) {
...
jQuery.fn.clearErrorAlert= function(){
  this.each(function(){
    ...
    jQuery("#"+el.attr("id")+"error").remove();
  });
  return this;
};
...
jQuery.fn.createErrorAlert = function(errorCode, message) {
  this.each(function() {
    ...
    if (jQuery("#" + el.attr("id") + "error").size() <= 0) {
...
function jsonErrorHandling(response){
  if (!response.ok){
    ...
    for (var i in response.clientEntryErrors){
      ...
      jQuery(":input[name="+err.name+"]").createErrorAlert(err.error);
    }
  }
}
...
jQuery(document).ready(function() {
  jQuery(":input[oldValue]").livequery(function(){jQuery(this).updateOldValue();});
  jQuery('.validated').livequery('blur', validateField);
});
...
function openBlackPopup(url,width,height,onCloseCallBack,iframeId){
  ...
  jQuery("#__blackpopup__").remove();

  var bg=jQuery("<div>").attr("id","__blackpopup__");
  ...
  var bdiv= jQuery("<div>").css({width:width,position:"relative",height:"5px", textAlign:"right", margin:"auto" });
...
function createBlackPage(width,height,onCloseCallBack){
  ...
  var bdiv= jQuery("<div>").css({width:width,position:"relative",height:"0px", textAlign:"right", margin:"auto" });
  var img=jQuery("<img src='res/closeBig.png' style='cursor:pointer;position:absolute;right:-40px;top:5px;' title='close'>");
  bdiv.append(img);
  img.click( function(){
    bg.trigger("close");
  });
...
function getBlackPopup(){
  var ret=jQuery("#__blackpopup__");
  if (typeof(top)!="undefined"){
    ret= window.parent.jQuery("#__blackpopup__");
  }
  return ret;
}
...
function openIssueEditorInBlack(issueId,command,params){
  ...
  openBlackPopup(editUrl,1020,jQuery(window).height()-50, function(){jQuery("#"+issueId).effect("highlight", { color: "yellow" }, 1500);});
}
...
function openBoardInBlack(boardId,command,params,callback){
  ...
  if (params)
    editUrl=editUrl+params;
  openBlackPopup(editUrl,jQuery(window).width()-100,jQuery(window).height()-50,callback );
}
