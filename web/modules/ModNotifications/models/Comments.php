...
class ModNotifications_CommentsModel {
	...
	function __construct($datarow) {
		...
		if (empty(self::$customerPortalUserId) && isModuleInstalled('CustomerPortal')) { // crmv@189136
			...
			//@nileio this is the root cause of the issue for returning a true 1 for user admin - where it should not return anything for admin user.
	...
	function authorPhoto() {
		$authorid = $this->data['smcreatorid'];
		//TODO: @nileio this call is somehow returning true where it should return false for notifications
		//resulting on user avatar not to be correctly returned. require more testing. it could be due to cache
		//commented out this as it returns true for admin user for now till i find out why $authorid == self::$customerPortalUserId ||
		// table is vte_customerportal_prefs which includes a query to get the user id for portal , returning 1 for admin user !
        // original if ($authorid == self::$customerPortalUserId || in_array($this->data['mod_not_type'],array('Ticket portal replied','Ticket portal created','Calendar invitation answer yes contact','Calendar invitation answer no contact'))) {
