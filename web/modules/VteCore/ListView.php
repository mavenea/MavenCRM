...
//custom code start
//@nileio damn it here is the code which checks what buttons appear on the list for contextual when you click on the ListView button
//@nileio we should always check if the module is active before listing it !
//@nileio added a check for moduleactive (email) & moduleactive(fax)
if(in_array($currentModule,array('Leads','Accounts','Contacts','Vendors'))){ // crmv@43765
	if (isPermitted('Emails', 'EditView', '') == 'yes' && vtlib_isModuleActive('Emails'))
		$list_buttons['s_mail'] = $app_strings['LBL_SEND_MAIL_BUTTON'];
	//@nileio damn this one here should also include check if the module is active // added
	if (isPermitted('Fax', 'EditView', '') == 'yes' && vtlib_isModuleActive('Fax'))
...
$customView = CRMEntity::getInstance('CustomView', $currentModule); // crmv@115329
$viewid = $customView->getViewId($currentModule);
//@nileio retrieve the list of custom views for the user as a dropdown html options.
...
//@nileio here the variable ALL is explicitly set to a value All if the selected view value of the combo represent the default.
//@nileio with my changes I am not using this anymore because I depend on key optionAll=boolean to determine if an option is the All option
...
$smarty->assign('BUTTONS', $list_buttons); //@nileio notice how you always must have a variable matching the smarty template.
...
//@nileio I am not depending on this variable anymore
