...
//@nileio this is only called when the application is first installed - reset opcache so that new symbols can be loaded from now on.
...
if (!isset($root_directory)) {
    ...
    if (!isset($root_directory)) die('root directory is not set or config.inc.php file is not loaded.'); //@nileio
...
if ($_REQUEST['morph_mode'] == 'installation' || VteSession::get('morph_mode') == 'installation') {
    ...
    if ($enterprise_mode == 'VTENEXTCE') { // crmv@192073
        ...
        if (function_exists('opcache_invalidate')) {
            opcache_invalidate($priv_file, true);
        }
    } // crmv@192073
}
$update_mode = false;
