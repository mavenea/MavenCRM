...
#
# Version 1.44
#  . Skip check of additional languages
#  . Fix php warning
#
# Version 1.43
#  . Force license check after update
#
# Version 1.42
#  . Fix warning during envcheck
#
# Version 1.41
#  . Fix http checkswith unreachable urls
#
# Version 1.40
#  . Fix label
#
# Version 1.39
#  . Fix for mysqldump 5.7.31
#
# Version 1.38
...
#
# Version 1.37
#  . Add check of php and apache settings
#
# Version 1.36
...
VTENEXT_VTE="https://partner.vtecrm.net/"
...
# check if a php extension is installed and loaded (via CLI)
check_php_ext_cli() {
	php -m | grep -q -F "$1" || return 1
	return 0
}

# check if a php extension is installed and loaded (via HTTP)
check_php_ext_web() {
	debug "Checking php extension $1..."
	echo '<?php echo extension_loaded($_REQUEST["ext"]) ? 1 : 0;' >"$VTEDIR""$ENVCHECKFILE"
	local RES=$(wget --quiet --tries=1 --timeout=2 -O - "$VTEURL/""$ENVCHECKFILE""?ext=$1" || echo 2)
	rm -f "$VTEDIR""$ENVCHECKFILE"
	# if empty string, errors with apache config
	if [ -z "$RES" -o "$RES" = "1" ]; then
		return 0
	elif [ "$RES" = '2' ]; then
		# timeout or network error
		warning "Unable to contact $VTEURL"
		return 2
	else
		return 1
	fi
...
envcheck_phpmods() {
	...
	local PHPMANDCHECK=0
	local PHPRECCHECK=0
	local PHPVERSION=$(php -r "echo phpversion();")

	if version_lt "$PHPVERSION" "7.0"; then
		# php < 7.0
		debug "Checking PHP modules not supported for PHP 5"
	else
		# php >= 7.0

		# check for mandatory modules
		local PHPMANDMODS="json xml mbstring curl imap gd zip"
		# add mysql is needed
		extract_php_global_var "${VTEDIR}config.inc.php" "dbconfig[.db_type.]"
		DBTYPE=$_RET
		if [ "$DBTYPE" = "mysql" -o "$DBTYPE" = "mysqli" ]; then
			PHPMANDMODS="$PHPMANDMODS mysqli"
		fi

		SKIPHTTPCHECK=0
		for PMOD in $PHPMANDMODS; do
			check_php_ext_cli $PMOD || {
				warning "PHP $PMOD module not installed or not active (via CLI)"
				PHPMANDCHECK=1
			}
			check_php_ext_web $PMOD || {
				if [ $? -eq 2 ]; then
					# network error, don't tryo othr modules
					warning "Unable to contact VTE, skipping HTTP checks"
					SKIPHTTPCHECK=1
					break
				fi
				warning "PHP $PMOD module not installed or not active (via HTTP)"
				PHPMANDCHECK=1
			}
		done

		# check for recommended modules
		local PHPRECMODS="bcmath apcu"
		for PMOD in $PHPRECMODS; do
			check_php_ext_cli $PMOD || {
				warning "PHP $PMOD module is recommended, but it's not installed or not active (via CLI)"
				PHPRECCHECK=1
			}
			if [ $SKIPHTTPCHECK -eq 0 ]; then
				check_php_ext_web $PMOD || {
					if [ $? -eq 2 ]; then
						# network error, don't tryo othr modules
						warning "Unable to contact VTE, skipping HTTP checks"
					...
					warning "PHP $PMOD module is recommended, but it's not installed or not active (via HTTP)"
					PHPRECCHECK=1
				}
			fi
		done
	...
	if [ $PHPMANDCHECK -eq 1 -a $BATCH -eq 0 ]; then
		ask "Some necessary PHP modules are missing. Do you want to continue anyway?" "Y/N"
		if [ "$_RET" = 'n' ]; then
...
envcheck_phpini() {
	...
	# Check PHP ini settings (CLI)
	_RET=$(php -r 'echo ini_get("error_reporting") ^ (E_ALL & ~E_NOTICE & ~E_STRICT & ~E_DEPRECATED);')
	if [ "$_RET" != "0" ]; then
		PHPINICHECK=1
		warning "PHP error_reporting is not set to E_ALL & ~E_NOTICE & ~E_STRICT & ~E_DEPRECATED (via CLI)"
	fi

	# enabled in CLI
	_RET=$(php -r 'echo ini_get("display_errors");')
	if [ "$_RET" != "1" ]; then
		PHPINICHECK=1
		warning "PHP display_errors should be set to 1 (via CLI)"
	fi

	# min 128MB
	_RET=$(php -r '$pows = array("k" => 1, "m" => 2, "g" => 3); $mem = ini_get("memory_limit"); $bytes = (int)$mem * pow(1024, $pows[strtolower(substr($mem, -1))]) ?: 0; echo $bytes;')
	if [ "$_RET" -gt 0 -a "$_RET" -lt 134217728 ]; then
		PHPINICHECK=1
		warning "PHP memory_limit is set to less than 128MB (via CLI)"
	fi

	# utf8 charset
	_RET=$(php -r 'echo ini_get("default_charset");')
	if [ "$_RET" != "UTF-8" ]; then
		PHPINICHECK=1
		warning "PHP default_charset is not UTF-8 (via CLI)"
	fi

	# Check PHP ini settings (HTTP)
	if [ $SKIPHTTPCHECK -eq 1 ]; then
		return
	fi

	echo '<?php echo ini_get("error_reporting") ^ (E_ALL & ~E_NOTICE & ~E_STRICT & ~E_DEPRECATED);' >"$VTEDIR""$ENVCHECKFILE"
	_RET=$(wget --quiet --tries=1 --timeout=2 -O - "$VTEURL/""$ENVCHECKFILE")
	# in case of empty answer, we are probably not calling vte
	if [ -z "$_RET" ]; then
		warning "Unable to access VTE via HTTP, proably a Webserver misconfiguration"
		rm -f "$VTEDIR""$ENVCHECKFILE"
		return
	fi
	...
	if [ "$_RET" != "0" ]; then
		PHPINICHECK=1
		warning "PHP error_reporting is not set to E_ALL & ~E_NOTICE & ~E_STRICT & ~E_DEPRECATED (via HTTP)"
	fi

	echo '<?php echo ini_get("display_errors");' >"$VTEDIR""$ENVCHECKFILE"
	_RET=$(wget --quiet --tries=1 --timeout=2 -O - "$VTEURL/""$ENVCHECKFILE")
	if [ "$_RET" != "0" ]; then
		PHPINICHECK=1
		warning "PHP display_errors should be 0 for production environments (via HTTP)"
	fi

	echo '<?php $mem = ini_get("memory_limit"); $bytes = (int)$mem * 1024 ** ["k" => 1, "m" => 2, "g" => 3][strtolower(substr($mem, -1))] ?? 0; echo $bytes;' >"$VTEDIR""$ENVCHECKFILE"
	_RET=$(wget --quiet --tries=1 --timeout=2 -O - "$VTEURL/""$ENVCHECKFILE")
	if [ "$_RET" -gt 0 -a "$_RET" -lt 134217728 ]; then
		PHPINICHECK=1
		warning "PHP memory_limit should be at least 128MB (now it's $_RET bytes, via HTTP)"
	fi

	echo '<?php echo ini_get("max_execution_time");' >"$VTEDIR""$ENVCHECKFILE"
	_RET=$(wget --quiet --tries=1 --timeout=2 -O - "$VTEURL/""$ENVCHECKFILE")
	if [ "$_RET" -gt 0 -a "$_RET" -lt 300 ]; then
		PHPINICHECK=1
		warning "PHP max_execution_time should be at least 300 (now it's $_RET, via HTTP)"
	fi

	echo '<?php echo ini_get("max_input_time");' >"$VTEDIR""$ENVCHECKFILE"
	_RET=$(wget --quiet --tries=1 --timeout=2 -O - "$VTEURL/""$ENVCHECKFILE")
	if [ "$_RET" -gt 0 -a "$_RET" -lt 60 ]; then
		PHPINICHECK=1
		warning "PHP max_input_time should be at least 60 (now it's $_RET, via HTTP)"
	fi

	echo '<?php echo ini_get("max_input_vars");' >"$VTEDIR""$ENVCHECKFILE"
	_RET=$(wget --quiet --tries=1 --timeout=2 -O - "$VTEURL/""$ENVCHECKFILE")
	if [ -n "$_RET" ]; then
		if [ "$_RET" -gt 0 -a "$_RET" -lt 5000 ]; then
			PHPINICHECK=1
			warning "PHP max_input_vars should be at least 5000 (now it's $_RET, via HTTP)"
		fi
	fi

	echo '<?php echo ini_get("file_uploads");' >"$VTEDIR""$ENVCHECKFILE"
	_RET=$(wget --quiet --tries=1 --timeout=2 -O - "$VTEURL/""$ENVCHECKFILE")
	if [ -n "$_RET" ]; then
		if [ "$_RET" != "1" ]; then
			PHPINICHECK=1
			warning "PHP file_uploads should be 1 (via HTTP)"
		fi
	fi

	echo '<?php die(""); $mem = ini_get("upload_max_filesize"); $bytes = (int)$mem * 1024 ** ["k" => 1, "m" => 2, "g" => 3][strtolower(substr($mem, -1))] ?? 0; echo $bytes;' >"$VTEDIR""$ENVCHECKFILE"
	_RET=$(wget --quiet --tries=1 --timeout=2 -O - "$VTEURL/""$ENVCHECKFILE")
	if [ -n "$_RET" ]; then
		if [ "$_RET" -gt 0 -a "$_RET" -lt 5242880 ]; then
			PHPINICHECK=1
			warning "PHP upload_max_filesize should be at least 5MB (now it's $_RET bytes, via HTTP)"
		fi
	fi

	echo '<?php $mem = ini_get("post_max_size"); $bytes = (int)$mem * 1024 ** ["k" => 1, "m" => 2, "g" => 3][strtolower(substr($mem, -1))] ?? 0; echo $bytes;' >"$VTEDIR""$ENVCHECKFILE"
	_RET=$(wget --quiet --tries=1 --timeout=2 -O - "$VTEURL/""$ENVCHECKFILE")
	if [ -n "$_RET" ]; then
		if [ "$_RET" -gt 0 -a "$_RET" -lt 8388608 ]; then
			PHPINICHECK=1
			warning "PHP post_max_size should be at least 8MB (now it's $_RET bytes, via HTTP)"
		fi
	fi
...
envcheck_apache() {
	...
	# check follow sym links
	local LINKFILE="envCheck_link.php"
	rm -f "$VTEDIR""$ENVCHECKFILE"
	rm -f "$VTEDIR""$LINKFILE"

	echo '<?php echo "LINKCHECK"; ' >"$VTEDIR""$ENVCHECKFILE"
	ln -s "$ENVCHECKFILE" "$LINKFILE"

	_RET=$(wget --quiet --tries=1 --timeout=2 -O - "$VTEURL/$LINKFILE")
	if [ "$_RET" != "LINKCHECK" ]; then
		APACHECHECK=1
		warning "Apache2 option FollowSymLinks should be activated"
	fi
	rm -f "$LINKFILE"
	rm -f "$VTEDIR""$ENVCHECKFILE"

	# check directory listing
	local LISTDIR="envCheck_dir"
	local LISTFILE="envCheck_dir/listing_file.txt"

	mkdir -p "$VTEDIR/$LISTDIR"
	echo "TEST" >"$VTEDIR/$LISTFILE"
	wget --quiet --tries=1 --timeout=2 -O - "$VTEURL/$LISTDIR" | grep -q -F "listing_file" && {
		APACHECHECK=1
		warning "Directory listing seems to be active, please check Apache2 configuration"
	}
	rm -rf "$VTEDIR/$LISTDIR"

	# check AllowOverride (not 100% safe and depending on mod headers)
	mkdir -p "$VTEDIR/$LISTDIR"
	echo 'Deny From All' >"$VTEDIR/$LISTDIR/.htaccess"
	echo '<?php /* do nothing */ ' >"$VTEDIR/$LISTDIR/envcheck.php"
	local RETCODE=$(wget --quiet --tries=1 --timeout=2 -S -O - "$VTEURL/$LISTDIR/envcheck.php" 2>&1 | grep -o -E "HTTP/\S* ([0-9]+)" | cut -d ' ' -f 2)
	if [ "$RETCODE" != "403" ]; then
		APACHECHECK=1
		warning "AllowOverride directive should set to All, please check Apache2 configuration"
	fi
	rm -rf "$VTEDIR/$LISTDIR"
...
envcheck() {
	...
	if [ $SKIPENVCHECK -eq 1 ]; then
		info "Environment check skipped as specified on command line"
		return
	fi

	debug "Checking environment..."

	# check if we have dest revision
	if [ $DESTREVISION -eq 0 ]; then
		error "You must specify a destination revision with the -d option"
		exit $RETCODE_FAIL
	fi
	...
	if [ $DESTREVISION -ge 1819 ]; then
		...
		version_lt "$PHPVERSION" "$MINPHPVERSION" && {
			error "The destination revision supports only PHP >= $MINPHPVERSION. Please update the system."
			exit $RETCODE_FAIL
		}
	fi

	envcheck_phpmods $DESTREVISION
	envcheck_phpini $DESTREVISION
	envcheck_apache $DESTREVISION
}
...
create_db_backup() {
	...
	mysqldump --default-character-set=utf8 --no-tablespaces --single-transaction --quick --routines --triggers --events --add-drop-table \
...
// force license check
if (is_file('cache/sys/mIiTtC.json')) @unlink('cache/sys/mIiTtC.json');
if (is_file('cache/sys/mIiTtC.php')) @unlink('cache/sys/mIiTtC.php');
...
vtews_call() {
	...
	debug "Calling webservice on ${VTENEXT_VTE}: $WSNAME"

	local OUTPUT=$(wget --quiet --timeout 20 "${VTENEXT_VTE}webservice.php?operation=$WSNAME" --post-data="$data" -O -)
...
send_revision_info() {
	local LICENSEID=$(cd "$VTEDIR"/modules/Morphsuit && php GetId.php 2>/dev/null)
	if [ -n "$LICENSEID" ]; then
		extract_php_global_var "$VTEDIR"config.inc.php application_unique_key
		local APPKEY="$_RET"
		extract_php_global_var "$VTEDIR""$VERSIONFILE" enterprise_current_build
		local ACTUALREV="$_RET"
		local ENCURL=$(php -r 'echo urlencode($argv[1]);' "$VTEURL")
		local data="method=trackVTEInfo&id=$LICENSEID&application_unique_key=$APPKEY&site_url=$ENCURL&revision=$ACTUALREV"
		debug "Sending revision information"
		wget --quiet --timeout 5 "${VTENEXT_VTE}modules/MorphsuitServer/Webservices.php" --post-data="$data" -O /dev/null 2>/dev/null
		# in case you want curl
		# curl -s -X POST "${VTENEXT_VTE}modules/MorphsuitServer/Webservices.php" -d "$data"
	else
		debug "No active license found to send revision information"
	fi
}
...
Options
    ...
    -r N, --rollback=N        Try to rollback VTE to revision N (only if a backup was previously created)
          ...
          --only-envcheck     Check only if the environment (php / apache) is suitable for the update
          ...
          --skip-envcheck     Do not check the environment before the update
