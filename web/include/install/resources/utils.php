...
class Installation_Utils
{
    ...
    static function getInstallableCoreModules()
    {
        require_once ('install/installable_modules.php');
        if (isset($installable_modules))
        {
            sort($installable_modules,SORT_STRING);
            return $installable_modules;
        } else return array();

    }
    static function getMandatoryCoreModules()
    {
        require_once ('install/installable_modules.php');
        if (isset($non_installable_modules))
        {
            return $non_installable_modules;
        } else return array();
            ...
            if ($result_conn) {
                ...
                if ($conn->Connect($db_hostname, $db_username, $db_password, $db_name)) {
                    ...
                    if (!$db_utf8_support) {
                        ...
                        // BUG: found by @nileio on June2021 - error undefined function check_db_utf8_support
                        // BUG FIX: changed to self::check_db_utf8_support($conn) and changed to a static function
    ...
    //BUG FIX: change to a static function
    //FIX: @nileio 2-Jun-21 changed to a static function
    ...
    //@nileio
    /**
     * Logs the installation progress to a file name install_progress.txt .
     *
     * @param string|null $msg The message to log to the file.
     * @param string|null $update (Optional) whether to append to previous line.
     *
     * @return bool|null True if message logged successfully, false if failed, or null if nothing is logge to
     * the file or cannot write to the file.
     * @todo error handling for creating the file.
     *
     */

    static function logInstallationProgress(?string $msg, ?bool $update = false): ?bool
    {

        $progressfile = 'install/_install_progress.txt'; //note relative to root
        if (isset($msg)) {
            try {
                if (!file_exists($progressfile)) {
                    return null;
                    //exit('progress file does not exist.');
                }
                $fhandle = fopen($progressfile, 'c+',); //open the file in seek mode for read and write
                if ($fhandle) {
                    $success=false;
                    if (filesize($progressfile) == 0) // if the file is empty we cannot be updating a row
                        $update = false;
                    if ($update) {
                        if (fseek($fhandle, -5, SEEK_END) == 0) { //hard coded -5 is the number of chars for </tr>
                            if (fwrite($fhandle, '<td class="cell-vcenter">' . $msg . '</td>' . PHP_EOL . '</tr>') !== FALSE) {
                                $success=true;
                            } else
                                $success=false;
                        }
                        ...
                        if (fseek($fhandle, 0, SEEK_END) == 0) {
                            if (fwrite($fhandle, '<tr>' . PHP_EOL . '<td class="cell-vcenter">' . $msg . '</td>' . PHP_EOL . '</tr>') !== FALSE) {
                                  //fflush($fhandle);
                                $success=true;
                            } else
                                $success=false;
                    ...
                    clearstatcache(true,   $progressfile ); //required to ensure that filesize function returns correct info.
                    fclose($fhandle);
                    return $success;
                } else
                    return null;
...
class ConfigFile_Utils
{
    ...
    //crmv@fix connection string end
    //@nileio this is where the config.inc.php is created very important
    function createConfigFile()
    {
        ...
        if ($templateHandle && $includeHandle) {
            //@nileio those calls should fix an issue where the updated config file is not loaded
            //updates the timestamp and enables next calls to the file to obtain updated meta data.
            clearstatcache(true,   $includeFilename );
            //if opcache is enabled and opcache.validate_timestamps is disabled (enabled by default on 7.4)
            //it is disabled on some systems such as heroku and therefore this call is essential in order to re-cache the new file.
            if (function_exists('opcache_invalidate')) {
                opcache_invalidate('config.inc.php', true);
                //now include the updated symbols for later use.
...
class Common_Install_Wizard_Utils
{
    ...
    //crmv@add mssql support end
    static function check_phpcli(): bool
    {
    $executable = 'php -v';
        $out = array();
        $ret = 0;
        exec($executable, $out, $ret);
        if ($ret == 0) {
            return true;
        }
        return false;
    ...
    public static function installMandatoryModules($skip_modules = array())
    {
        ...
        //@nileio suppress warning error message for directory not existing
    ...
    public static function renameInstallationFiles()
    {
        ...
        //we delete it here because at this point the file must just be empty
        $ins_errorlog_removed = unlink('install/install_errlog.txt');
        //we delete it the install request file because it contains sensitive information
        $ins_jobrequest_removed= unlink('install/_install_request.inc.php');
        ...
        //@nileio
        $result['install_errorlog_removed'] = $ins_errorlog_removed;
        $result['install_jobrequest_removed'] = $ins_jobrequest_removed;

        return $result;
    }

    public static function startCronSleep(string $rootdir): bool
    {
        //@nileio workaround for heroku or heroku-like environments
        //run cron as a sleep job
        if (!isset($rootdir)) return false;

        $execfile = $rootdir . 'cron/RunSleep.sh';
        $cmd =$execfile .' >/dev/null 2>&1 &';
        $out=shell_exec($cmd);
        if ($out)
        return false;
            else
    ...
    /*	//crmv@fix hostname
        public static function constructHostname($dbtype, $hostname, $port) {
            //@nileio remove this check for now. it doesnt work with non-default mysql port
            //	if ($dbtype == 'mysqli' || $dbtype == 'mssqlnative') { // crmv@155585
            //@nileio removed check for mysqli to work with non-standard ports correctly.
            //assumes version of adodb installed with vte 20
            if ($dbtype == 'mssqlnative') { // crmv@155585
                return $hostname;
            } else {
