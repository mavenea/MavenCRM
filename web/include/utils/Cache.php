...
class Cache {
	...
	function __construct($name, $extension=null, $rootFolder=null) {
		...
		// crmv@115378
		$this->enabled = PerformancePrefs::getBoolean('CACHE'); //enabled by default
		$this->type = PerformancePrefs::get('CACHE_TYPE');  // session by default
		// crmv@115378e
		
		if (in_array($name,array('vteCacheHV','mIiTtC','numberUsersMorphsuit','cacheResources'))) $this->type = 'file'; //crmv@61417 crmv@94125
		
		if ((basename($_SERVER['PHP_SELF']) == 'install.php') || (basename($_SERVER['PHP_SELF']) == 'RunInstall.php')) $this->enabled = false;
