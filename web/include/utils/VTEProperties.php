...
class VTEProperties extends VTEUniqueClass {
    ...
    public function __construct() {
        ...
        //@nileio
        array_walk_recursive($this->default_values,array($this, 'replace_envvars'));
        //@nileio encrypt passwords for rabbitmq
        $rabbitmqEnabled = $this->default_values['performance.mq_rabbitmq_enabled'];
        $stompEnabled = $this->default_values['performance.mq_webstomp_enabled'];
        if ($rabbitmqEnabled){
            $rabbitmqConnection = $this->default_values['performance.mq_rabbitmq_connection'];
            require_once('include/utils/encryption.php');
            $encryption = new Encryption();
            $this->default_values['performance.mq_rabbitmq_connection']['password'] = $encryption->encrypt($rabbitmqConnection['password']);
        }
        if ($stompEnabled){
            $stompConnection = $this->default_values['performance.mq_webstomp_connection'];
            require_once('include/utils/encryption.php');
            $encryption = new Encryption();
            $this->default_values['performance.mq_webstomp_connection']['password'] = $encryption->encrypt($stompConnection['password']);
        }
    ...
    protected function isInstallRunning() {
        //@nileio add RunInstall.php
        return (basename($_SERVER['PHP_SELF']) == 'install.php' || basename($_SERVER['PHP_SELF']) == 'RunInstall.php');
    }
    ...
    // crmv@164465e
    //@nileio new function to get php session info
    protected function getPHPSessionHandler(&$config){
    ...
    //@nileio
    // function to replace environment variables in the format ${var_name} with the value from environment if found.
    // value is replaced by reference so nothing to return. no change is made if variable can't be resolved.
    protected function replace_envvars(&$val) {
        if(preg_match_all('/\${(.[^\$][^{]*?)}/',$val,$matches)){
            for ($i = 0; $i < count($matches[1]); $i++) {
                $var = getenv($matches[1][$i]);
                if ($var)
                    $val=str_replace($matches[0][$i],$var,$val);
            }
        }
    }
}