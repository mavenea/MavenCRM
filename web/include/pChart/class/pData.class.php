 ...
 class pData
  {
   ...
   function addPoints($Values,$SerieName="Serie1")
    ...
    // crmv@30014 - stretch palette to match the number of values
    function stretchPalette($SerieName = 'Serie1') {
    	$dataCount = count($this->Data["Series"][$SerieName]["Data"]);
    	$paletteCount = count($this->Palette);

    	if ($dataCount <= 1 || $paletteCount <= 1) return;

    	$Palette2 = array();
    	for ($I=0; $I<$dataCount; ++$I) {
    		$exactPos = $I * ($paletteCount-1) / ($dataCount-1);
    		$P1 = intval(floor($exactPos));
    		$P2 = $P1+1;
    		$fract = 1.0 - ($exactPos - $P1);
    		$MidColor = array(
    			'R' => ($this->Palette[$P1]['R'] * $fract + $this->Palette[$P2]['R'] * (1.0 - $fract)),
    			'G' => ($this->Palette[$P1]['G'] * $fract + $this->Palette[$P2]['G'] * (1.0 - $fract)),
    			'B' => ($this->Palette[$P1]['B'] * $fract + $this->Palette[$P2]['B'] * (1.0 - $fract)),
    			'Alpha' => ($this->Palette[$P1]['Alpha'] * $fract + $this->Palette[$P2]['Alpha'] * (1.0 - $fract)),
    		);
    		$Palette2[$I] = $MidColor;
    		//$Palette2[$I] = $this->Palette[ round($I * ($paletteCount-1) / ($dataCount-1)) ];
    	}
   ...
   function convertToArray($Value)
    { $Values = array(); $Values[] = $Value; return($Values); } // crmv@167234
