 ...
 class pImage extends pDraw
  {
   ...
   /* Return the surrounding box of text area */
   // crmv@31696 - fix legend overlapping for multiple lines
   function getTextBox($X,$Y,$FontName,$FontSize,$Angle,$Text)
    {
    // fix for multiline text
     $Text = str_replace("\r", "", $Text);
     $Lines = explode("\n", $Text);
     $coords = imagettfbbox($FontSize, 0, $FontName, $Lines[0]);
     $lineheight = $coords[1] - $coords[7];
     for($i=1; $i<count($Lines); ++$i) {
     	$tc = imagettfbbox($FontSize, 0, $FontName, $Lines[$i]);
     	$coords[0] = min($coords[0], $tc[0]);
     	//$coords[1] += $tc[1];
     	$coords[2] = max($coords[2], $tc[2]);
     	//$coords[3] += $tc[3];
     	$coords[4] = max($coords[4], $tc[4]);
     	$coords[5] += $tc[5];
     	$coords[6] = min($coords[6], $tc[6]);
     	$coords[7] += $tc[7];
     }
     ...
     for($i = 0; $i < 7; $i += 2)
      {
       ...
       $RealPos[$i/2]["Y"] = $Y + $lineheight * (count($Lines) - 1) + round($coords[$i+1] * $ca - $coords[$i] * $sa);
      }
   ...
   function addToImageMap($Type,$Plots,$Color=NULL,$Title=NULL,$Message=NULL,$HTMLEncode=FALSE, $Percentage=FALSE)
    {
     if ( $this->ImageMapStorageMode == NULL ) { $this->initialiseImageMap(); }

     /* Encode the characters in the imagemap in HTML standards */
     $Title   = str_replace("&#8364;","\u20AC",$Title);
     $Title   = htmlentities($Title,ENT_QUOTES,"UTF-8"); // crmv@31681
     if ( $HTMLEncode )
      {
       $Message = htmlentities($Message,ENT_QUOTES,"UTF-8"); // crmv@31681
     ...
     elseif($this->ImageMapStorageMode == IMAGE_MAP_STORAGE_FILE)
      {
       ...
       fwrite($Handle, $Type.IMAGE_MAP_DELIMITER.$Plots.IMAGE_MAP_DELIMITER.$Color.IMAGE_MAP_DELIMITER.$Title.IMAGE_MAP_DELIMITER.$Message.IMAGE_MAP_DELIMITER.$Percentage."\r\n"); // crmv@30014
       fclose($Handle);
      }
    }
       ...
       if ($Handle)
        {
         while (($Buffer = fgets($Handle, 4096)) !== false)
          {
           $Fields      = explode(IMAGE_MAP_DELIMITER,str_replace(array(chr(10),chr(13)),"",$Buffer)); // crmv@146653
   ...
   function toHTMLColor($R,$G,$B)
    {
     //crmv@30014
     if (is_string($R) && stripos($R, 'x') !== false)
     	$R = hexdec($R);
     else
     	$R=intval($R);
     if (is_string($G) && stripos($G, 'x') !== false)
     	$G = hexdec($G);
     else
     	$G=intval($G);
     if (is_string($B) && stripos($B, 'x') !== false)
     	$B = hexdec($B);
     else
     	$B=intval($B);
     //crmv@30014
