   ...
   function drawRoundedFilledRectangle($X1,$Y1,$X2,$Y2,$Radius,$Format="")
    {
     ...
     $Step = 360 / (2 * PI * $Radius);
     $Positions = array(); $Radius--; $MinY = ""; $MaxY = ""; // crmv@167234
     for($i=0;$i<=90;$i=$i+$Step)
      {
   ...
   function drawArrowLabel($X1,$Y1,$Text,$Format="")
    {
     ...
     $Ticks    = isset($Format["Ticks"]) ? $Format["Ticks"] : NULL;
     $DrawLine = isset($Format["DrawLine"]) ? $Format["DrawLine"] : TRUE; // crmv@30014
     $DrawArrow = isset($Format["DrawArrow"]) ? $Format["DrawArrow"] : TRUE; // crmv@30014
     ...
     if ($DrawArrow) {
     	...
     	$this->drawLine($X2,$Y2,$X1,$Y1,$Format);
     }
     // crmv@30014e
   ...
   function drawLegend($X,$Y,$Format="")
    {
     ...
     $Boundaries = array(); $Boundaries["L"] = $X; $Boundaries["T"] = $Y; $Boundaries["R"] = 0; $Boundaries["B"] = 0; $vY = $Y; $vX = $X; // crmv@167234
     foreach($Data["Series"] as $SerieName => $Serie)
      {
       if ( $Serie["isDrawable"] == TRUE && $SerieName != $Data["Abscissa"] )
        {
     ...
     if ( $GotXAxis == FALSE )
      {
       if ( $Abscissa != NULL )
        {
         $Points = is_array($Data["Series"][$Abscissa]["Data"]) ? count($Data["Series"][$Abscissa]["Data"]) : 0; // crmv@172864
         if ( $AutoAxisLabels )
   ...
   function computeScale($XMin,$XMax,$MaxDivs,$Factors,$AxisID=0)
    {
     ...
     $GoodScaleFactors = array(); // crmv@167234
   ...
   function scaleComputeY($Values,$Option="",$ReturnOnly0Height=FALSE)
    {
     ...
     if ( !is_array($Values) ) { $tmp = $Values; $Values = array(); $Values[0] = $tmp; } // crmv@167234

     $Result = array(); // crmv@167234
     if ( $Data["Orientation"] == SCALE_POS_LEFTRIGHT )
      {
           ...
           foreach($PosArray as $Key => $Y)
			...
			// crmv@30014 - switch pos
             ...
             if ( $Y != VOID && $LastY == NULL && $LastGoodY != NULL && !$BreakVoid )
              ...
              {
              	if ( $Serie["Data"][$Key] > 0 ) {
              		$Align = TEXT_ALIGN_BOTTOMMIDDLE; $Offset = $DisplayOffset;
              	} else { $Align = TEXT_ALIGN_TOPMIDDLE; $Offset = -$DisplayOffset;
              	}
     ...
     if ( $OverrideColors != NULL )
      {
       ...
       $PaletteOrigSize = count($OverrideColors); // crmv@30014
      }
       ...
       if ( $Serie["isDrawable"] == TRUE && $SerieName != $Data["Abscissa"] )
        {
         ...
         $SerieSum = $this->DataSet->getSum($SerieName); // crmv@30014
           ...
           foreach($PosArray as $Key => $Y2)
            {
             ...
             if ( $OverrideColors != NULL ) {
                ...
                } elseif ($CyclePalette && $PaletteOrigSize > 0) {
              	 $Color = array("R"=>$OverrideColors[$ID % $PaletteOrigSize]["R"],"G"=>$OverrideColors[$ID % $PaletteOrigSize]["G"],"B"=>$OverrideColors[$ID % $PaletteOrigSize]["B"],"Alpha"=>$OverrideColors[$ID % $PaletteOrigSize]["Alpha"],"BorderR"=>$OverrideColors[$ID % $PaletteOrigSize]["BorderR"],"BorderG"=>$OverrideColors[$ID % $PaletteOrigSize]["BorderG"],"BorderB"=>$OverrideColors[$ID % $PaletteOrigSize]["BorderB"]);
                } else {
             ...
             if ( $Y2 != VOID )
              {
               $BarHeight = $Y1 - $Y2;
               $MapValue = $this->scaleFormat($Serie["Data"][$Key],$Mode,$Format,$Unit); // crmv@30014
               if ( $Serie["Data"][$Key] == 0 )
                {
                 $this->drawLine($X+$XOffset+$XSpace,$Y1,$X+$XOffset+$XSize-$XSpace,$Y1,$Color);
                 if ( $RecordImageMap ) { $this->addToImageMap(
                 		"RECT",
                 		floor($X+$XOffset+$XSpace).",".floor($Y1-1).",".floor($X+$XOffset+$XSize-$XSpace).",".floor($Y1+1),
                 		$this->toHTMLColor($Color['R'],$Color['G'],$Color['B']),$SerieDescription, $MapValue,FALSE,round(( 100 / $SerieSum ) * $MapValue)); } // crmv@30014
                }
               else
                {
                 if ( $RecordImageMap ) { $this->addToImageMap("RECT",floor($X+$XOffset+$XSpace).",".floor($Y1).",".floor($X+$XOffset+$XSize-$XSpace).",".floor($Y2),$this->toHTMLColor($Color['R'],$Color['G'],$Color['B']),$SerieDescription,$MapValue, FALSE,round(( 100 / $SerieSum ) * $MapValue)); } // crmv@30014
               ...
               if ( $Serie["Data"][$Key] == 0 )
                {
                 ...
                 if ( $RecordImageMap ) { $this->addToImageMap("RECT",floor($X1-1).",".floor($Y+$YOffset+$YSpace).",".floor($X1+1).",".floor($Y+$YOffset+$YSize-$YSpace),$this->toHTMLColor($Color['R'],$Color['G'],$Color['B']),$SerieDescription,$MapValue,FALSE,round(( 100 / $SerieSum ) * $MapValue)); } // crmv@30014
                }
               else
                {
                 if ( $RecordImageMap ) { $this->addToImageMap("RECT",floor($X1).",".floor($Y+$YOffset+$YSpace).",".floor($X2).",".floor($Y+$YOffset+$YSize-$YSpace),$this->toHTMLColor($Color['R'],$Color['G'],$Color['B']),$SerieDescription,$MapValue, FALSE,round(( 100 / $SerieSum ) * $MapValue)); } // crmv@30014
