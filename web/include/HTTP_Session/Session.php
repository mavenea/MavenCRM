...
class HTTP_Session
{
    ...
    function setContainer($container, $container_options = null)
    {
        ...
        $container_classfile = 'include/HTTP_Session/Session/Container/' . $container . '.php';
    ...
    function start($name = 'SessionID', $id = null)
    {
       ...
       // session_id($id); //@nileio this is already set in the previous calls
        if (!VteSession::getId()) VteSession::start();  //@nileio BUGFIX: session is lost for ajax calls directly to webservice.php
        //CAUSE: webservice.php is called after the session is closed. session management should be with VTESession
        //this class and others handling the session is possibly redudunant but cant remove them now. This fixes the issue with BPMN etc.
        session_start();
    ...
    function useCookies($useCookies = null)
    {
        ...
        if (isset($useCookies)) {
            @ini_set('session.use_cookies', $useCookies ? 1 : 0); // crmv@172864
        }
        return $return;
    }
