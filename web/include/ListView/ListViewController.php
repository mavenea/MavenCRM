			...
			if ($module != 'Messages') {
				...
				//crmv@fix Calendar end
					if (isset($navigationInfo['start']) && $navigationInfo['start'] > 1) {
						$actionLinkInfo .= "<a href=\"$edit_link&start=".$navigationInfo['start']."\"><i class='vteicon mav-lv-icon-edit' title='".getTranslatedString("LBL_EDIT",$module)."'>create</i></a> ";
					} else {
						$actionLinkInfo .= "<a href=\"$edit_link\"><i class='vteicon mav-lv-icon-edit' title='".getTranslatedString("LBL_EDIT",$module)."'>edit</i></a> ";  //@nileio changed icon from create to edit
					}
				}
				if($deletePermission == 'yes'){ //crmv@179057
					...
					if($actionLinkInfo != "" && $del_link != "")
						$actionLinkInfo .=  "&nbsp;";
					if($del_link != "")
						$actionLinkInfo .=	"<a href='javascript:confirmdelete(\"".addslashes(urlencode($del_link))."\",\"".$module."\")'><i class='vteicon mav-lv-icon-del' title='".getTranslatedString("LBL_DELETE",$module)."' >delete</i></a>";	//crmv@144123 @nileio changed icon from clear to delete
				}
