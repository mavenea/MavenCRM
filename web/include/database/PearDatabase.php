...
class PearDatabase{
    ...
    var $dbType = null;
    var $dbHostName = null;
    var $dbPort = ''; //@nileio : add a dbPort variable
    var $dbName = null;
    var $dbOptions = null;
    var $userName=null;
    ...
    //@nileio
    function setDatabasePort($port){ $this->dbPort = $port;	}
    ...
    function checkConnection(){
		...
		if(!isset($this->database)) {
		    $this->println("checkConnection(): TRANS creating new connection");
		    $this->connect(false);
		} else {
...
//		$this->println("updateClob t=".$tablename." c=".$colname." id=".$id." status=".$string_result);
    ...
    function connect($dieOnError = false) {
      ...
      /*  if (isset($dbconfig)) {
            if(!isset($this->dbType) && $dbconfig != '' && $dbconfig['db_type'] != '_DBC_TYPE_') {
                $this->resetSettings($dbconfig['db_type'],'','','','');
                //$this->dbType= $dbconfig['db_type'];
            } elseif (!isset($this->dbType) && $dbconfig != '' && $dbconfig['db_type'] == '_DBC_TYPE_' && isset($_SESSION['config_file_info'])){
                $dbconfig['db_type'] = $_SESSION['config_file_info']['db_type'];
                $this->resetSettings($dbconfig['db_type'],'','','','');
            } else
            {
                require ('config.inc.php');
                if ($dbconfig['db_type']!=='_DBC_TYPE_')
                    $this->resetSettings($dbconfig['db_type'],'','','','');
            }
    ...
    //        $this->resetSettings('', '', '', '', '');
     //       if (!isset($this->dbType)) {
     //           $this->println("Connect() : cannot connect. DBType not specified. Returning.");
     //           return;
     //       }
     //   }
		...
		if ($this->usePersistent) {
			$this->database->PConnect($this->constructHostname($this->dbType,$this->dbHostName,$this->dbPort), $this->userName, $this->userPassword, $this->dbName);
		} else {
			$this->database->Connect($this->constructHostname($this->dbType,$this->dbHostName,$this->dbPort), $this->userName, $this->userPassword, $this->dbName);
		}
		// crmv@65455e
        ...
        if (isset($dbconfig) && $dbconfig['db_dieOnError'] === true){
	...
	if(!isset($this->dbType))
	{
	    $this->println("_construct() : DBType not specified. Returning.");
	    return;
	}
    ...
    function resetSettings($dbtype,$host,$dbname,$username,$passwd, $charset = ''){ // crmv@124454
	    //
      //  global $dbconfigoption,$dbconfig ;//,$dbconfig;
        // case 1: config.inc.php is empty
        // case 2: config.inc.php have values but should not be used yet
        // case 3: config.inc.php have values and should be used now
//        require ('config.inc.php');
      //  if (!isset($dbconfig))
        //    require ('config.php');
            //global $dbconfig;

/*        if ($dbconfig['db_type']=='_DBC_TYPE_' && isset($_SESSION['config_file_info'])) {
            $configFileUtils = new ConfigFile_Utils($_SESSION['config_file_info']);
            $dbconfig['db_server'] = $configFileUtils->getDbHostName();
            $dbconfig['db_port'] = $configFileUtils->getDbPort();
            $dbconfig['db_hostname'] = $dbconfig['db_server'] . $dbconfig['db_port'];
            $dbconfig['db_username'] = $configFileUtils->getDbUserName();
            $dbconfig['db_password'] = $configFileUtils->getDbPassword();
            $dbconfig['db_name'] = $configFileUtils->getDbName();
            $dbconfig['db_type'] = $configFileUtils->getDbType();
            $dbconfig['db_charset'] = $configFileUtils->getDbCharSet();
        }*/
        // Ok i think i found the problem
        //if (!isset($dbconfig) || empty($dbconfig['db_type']) || $dbconfig['db_type']=='_DBC_TYPE_')
        //{
            //i have no idea what and why is this even needed !
        //    copy('config.inc.php','tmp.inc.php');
        //    include_once ('tmp.inc.php');
        //}
        ...
        //            $this->dbType= $dbconfig['db_type'];
        //        } else {
        //            die('PearDatabase -> ResetSettings() : Db configuration cannot be loaded.');
        //        }
        //    }
    ...
    function createTables($schemaFile, $dbHostName=false, $userName=false, $userPassword=false, $dbName=false, $dbType=false) {
		$this->println("createTables ".$schemaFile);
		//@nileio I DONT KNOW WHY we need to have a new host/password/etc. here those cannot be used
        //if we do then we need to add the port in the parameters too
        //TODO: Check when this function is ever called with Params
		...
		$this->println("createTables ".$schemaFile." status=".$result);
		return $result;
    }

    //@nileio copied from     ////crmv@fix hostname
    function constructHostname($dbtype, $hostname, $port) {
    ...
    function createTable($tablename, $flds) {
		$this->println("createTable table=".$tablename." flds=".$flds);
		$this->checkConnection();
		$dict = NewDataDictionary($this->database);
		$sqlarray = $dict->CreateTableSQL($tablename, $flds);
		$result = $dict->ExecuteSQLArray($sqlarray);
		$this->println("createTable table=".$tablename." flds=".$flds." status=".$result);
		return $result;
    }

    function alterTable($tablename, $flds, $oper, $printError = true) { // crmv@150747
		$this->println("alterTableTable table=".$tablename." flds=".$flds." oper=".$oper);
		$this->checkConnection();
		$dict = NewDataDictionary($this->database);
