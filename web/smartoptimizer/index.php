...
// crmv@81852 - skip minify for some problematic files
if (is_array($settings['skip_minify']) && in_array($query, $settings['skip_minify'])) {
	$settings['minify'] = false;
}
// skip already minified files
if ($settings['skip_min_files'] && (substr($query, -7) == '.min.js' || substr($query, -8) == '.min.css')) {
	$settings['minify'] = false;
}
// crmv@81852e

//crmv@34832 - removed check
...
//crmv@154968
if (strpos($fileDir, 'modules/Touch/overrides/css_') !== false) {
	headerCORS();
}
//crmv@154968e
...
if (!$settings['clientCache'] || !$settings['clientCacheCheck'] || !isset($_SERVER['HTTP_IF_MODIFIED_SINCE']) || $_SERVER['HTTP_IF_MODIFIED_SINCE'] != $mtimestr) {
	...
	if ($generateContent) {
		...
		if ($settings['minify'] && $fileType == "css") {
			...
			#@nileio: using new minifer lib for css only. not using it for js because it produced some errors.
			$mode = ($settings['gzip'] ? 1 : 0) + ($settings['serverCache'] ? 2 : 0);
			$content = call_user_func('minify_css', $files, $mode, $cachedFile);
		} else {
		$content = array();
		foreach ($files as $file) (($content[] = @file_get_contents($file)) !== false) || debugExit("File not found ($file).");
		$content = implode("\n", $content);
			if ($settings['minify'] && $fileType == "js") $content = call_user_func('minify_js', $content);
