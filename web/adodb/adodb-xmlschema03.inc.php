...
if( !defined( 'XMLS_EXISTING_DATA' ) ) {
	define( 'XMLS_EXISTING_DATA', XMLS_MODE_UPDATE ); // crmv@fix
}
...
class dbTable extends dbObject {
	...
	*/
	function addTableOpt( $opt ) {
		if($this->currentPlatform) { // crmv@fix
...
class dbIndex extends dbObject {
	...
	*/
	function create( &$xmls ) {
		...
		//crmv@fix indexes
		if ($xmls->db->dataProvider == 'oci8'){
			$this->name = substr($this->name,0,29);
			foreach ($this->columns as $column)
				$columns_lower[] = strtolower($column);
			$array_compare = array();	
			foreach ($this->parent->fields as $arr){
				if (is_array($arr['OPTS'])){
					foreach ($arr['OPTS'] as $opt){
						if (is_array($opt)) continue;
						if (count($columns_lower) > 0 && strtoupper($opt) == 'KEY' && in_array(strtolower($arr['NAME']),$columns_lower)){
							$array_compare[] = strtolower($arr['NAME']);
						}
					}	
				}
			}	
			if (count(array_diff($columns_lower,$array_compare)) == 0)
				return;					
		}
		//crmv@fix indexes end
...
class dbData extends dbObject {
	...
	*/
	function create( &$xmls ) {
		...
		// eliminate any columns that aren't in the table
		// crmv@fix
		if ($xmls->db->dataProvider == 'mssql'){
			$table_fields = $this->parent->fields;
			$found = false;
			foreach ($table_fields as $field){
				if (in_array( 'AUTOINCREMENT', $field['OPTS'] )){
					$found = true;
					break;
				}	
			}	
		...
		foreach( $this->data as $row ) {
			...
			foreach( $row as $field_id => $field_data ) {
				...
				$rawfields[$name] = $field_data; // crmv@fix
				switch( $table_fields[$field_id]['TYPE'] ) {
					...
					case 'I8':
						...
						break;
					// crmv@fix
					case (in_array($table_fields[$field_id]['TYPE'],array('T','D')) && in_array($field_data,array('0000-00-00 00:00:00','0000-00-00'))):	
							$fields[$name] = "NULL";
						break;
					// crmv@fix-e
					case 'C':
			...
			$sql[] = 'INSERT INTO '. $table .' ('. implode( ',', array_keys( $xmls->db->sanitize_field_array($fields) ) ) .') VALUES ('. implode( ',', $fields ) .')'; // crmv@fix
				continue;
		}
		
			// Prepare to test for potential violations. Get primary keys and unique indexes
			$mfields = array_merge( $fields, $rawfields );
			if (is_array($ukeys)) // crmv@fix
			...
			if( empty( $ukeys ) or count( $keyFields ) == 0 ) {
				...
				$sql[] = 'INSERT INTO '. $table .' ('. implode( ',', array_keys( $xmls->db->sanitize_field_array($fields) ) ) .') VALUES ('. implode( ',', $fields ) .')';	//crmv@21249
				continue;
			}
			...
			$records = $xmls->db->Execute( 'SELECT * FROM ' . $table . $where_query);
			//crmv@21249e
...
class adoSchema {
	...
	*/
	function ParseSchemaString( $xmlstring, $returnSchema = FALSE ) {
		...
		if( !xml_parse( $xmlParser, $xmlstring, TRUE ) ) {
			die( sprintf(
				"XML error: %s at line %d at column %d", // crmv@fix
				xml_error_string( xml_get_error_code( $xmlParser) ),
				xml_get_current_line_number( $xmlParser),
				xml_get_current_column_number( $xmlParser) // crmv@fix
			) );
		}
	...
	*/
	...
	// crmv@fix
	function ExtractSchema_singletable($table,$data=false){
		return self::ExtractSchema($data,'  ','',false,$table);
	}
	// crmv@fix-e
	...
	*/
	// crmv@fix - function heavily modified to write to a temp file to save memory
	function ExtractSchema( $data = FALSE, $indent = '  ', $prefix = '' , $stripprefix=false,$table = false,$wheres=false,$noseq=false,$file=false) {
		global $table_prefix;
		
		$old_mode = $this->db->SetFetchMode( ADODB_FETCH_NUM );
		if (!$file){
			$file = tempnam(sys_get_temp_dir(), 'schema_dump');
			$return_content = true;
		}
		...
		if( is_array( $tables = $this->db->MetaTables( 'TABLES' , ($prefix) ? $prefix.'%' : '') ) ) {
			...
			foreach( $tables as $table ) {
				//skip sequence tables,handled separately
				if ($noseq && substr($table,-4) == '_seq')
					continue;
				$field_types = array();
				//crmv@30456
				$tablepref = $table;
				if(strpos($table, $table_prefix) !== false){
					$tablepref=str_replace($table_prefix.'_', 'TABLEPREFIX_', $table);
				}
				//crmv@30456e
					
				if ($stripprefix) $table = str_replace(str_replace('\\_', '_', $pfx ), '', $table);
				fwrite($file_handle,$indent . '<table name="' . htmlentities( $tablepref ) . '">' . "\n");	//crmv@30456 
				
				//crmv@get opt table (only 4 mysql)
				if (strtolower($this->db->databaseType) == 'mysql'){
					$rs = $this->db->Execute( 'SHOW CREATE TABLE '.$table);
					if (is_object( $rs ) && !$rs->EOF){
						...
						if ($mysql_opt){
							$split = explode("=",$mysql_opt);
							if (trim($split[0]) == 'ENGINE'){
								$engine = explode(" ",$split[1]);
								$mysql_opt_string = "ENGINE=".trim($engine[0]);
								fwrite($file_handle,$indent . "<opt platform=\"mysql\">$mysql_opt_string</opt>\r\n");
							}
						}	
					}
				}
				//crmv@get opt table (only 4 mysql) end
				
				// grab details from database
				$where ='';
				if ($wheres){
					if ($wheres[$table] && $wheres[$table]->condition != ''){
						$where = $wheres[$table]->condition;
					}
				}	
				...
				if( is_array( $fields ) ) {
					foreach( $fields as $details ) {
						...
						if( isset($details->max_length) && $details->max_length > 0 ) {
							//crmv@fix scale
							if (isset($details->scale) && $details->scale > 0)
								$size = $details->max_length.",".$details->scale;
							else
						...
						if( isset($details->primary_key) && $details->primary_key ) {
							$content[] = '<KEY/>';
						} elseif( isset($details->not_null) && $details->not_null && (!in_array($type,array('D','T'))) ) {
//							$content[] = '<NOTNULL/>';
						}
						
						if( isset($details->has_default) && $details->has_default && (!in_array($type,array('D','T')))) {
...
//							$content[] = '<AUTOINCREMENT/>';
						...
						$field_types[] = $type;
						fwrite($file_handle,str_repeat( $indent, 2 ) . '<field name="' . htmlentities( $details->name ) . '" type="' . $type . '"' . $extra);
				...
				if( is_array( $indexes ) ) {
					foreach( $indexes as $index => $details ) {
						...
						//crmv@30456 
						if(strpos($index, $table_prefix) !== false){
							$index=str_replace($table_prefix.'_', 'TABLEPREFIX_', $index);
						}
						//crmv@30456e
						fwrite($file_handle,str_repeat( $indent, 2 ) . '<index name="' . $index . '">' . "\n");
						
						if( $details['unique'] ) {
							fwrite($file_handle,str_repeat( $indent, 3 ) . '<UNIQUE/>' . "\n");
						}
						
						foreach( $details['columns'] as $column ) {
							fwrite($file_handle,str_repeat( $indent, 3 ) . '<col>' . htmlentities( $column ) . '</col>' . "\n");
						}
						
						fwrite($file_handle,str_repeat( $indent, 2 ) . '</index>' . "\n");
					}
				}
				
				if( $data ) {
					$rs = $this->db->Execute( 'SELECT * FROM ' . $table .' '.$where);
					if( is_object( $rs ) && !$rs->EOF ) {
						fwrite($file_handle,str_repeat( $indent, 2 ) . "<data>\n");
						while( $row = $rs->FetchRow() ) {
							foreach( $row as $key => $val ) {
								if (in_array($field_types[$key],array('N','I','F')) && ($val === null || $val == ''))
									$row[$key] = 0;
							...
							fwrite($file_handle,str_repeat( $indent, 3 ) . '<row><f>' . implode( '</f><f>', $row ) . "</f></row>\n");
						}
				...
				fwrite($file_handle,$indent . "</table>\n");
			}
		}
		
		$this->db->SetFetchMode( $old_mode );
		
		fwrite($file_handle,'</schema>');
		fclose($file_handle);
		if ($return_content){
