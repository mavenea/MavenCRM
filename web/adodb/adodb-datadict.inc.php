...
class ADODB_DataDict {
	...
	function ExecuteSQLArray($sql, $continueOnError = true, $printError = true) // crmv@129940
	{
		$rez = 2;
		$conn = $this->connection;
		$saved = $conn->debug;
		$emptyCache = false;	//crmv@47905bis
		foreach($sql as $line) {
			...
			//crmv@47905bis
				...
				foreach($cache_emptiers as $ce) {
					if (stripos($line,$ce) === 0) $emptyCache = true;
				}
			}
			//crmv@47905bis e
		}
		...
		if ($emptyCache && !$globSkipClearCache) {
	...
	*/
	function CreateTableSQL($tabname, $flds, $tableoptions=array(),$temporary = false) // crmv@fix
	{
		...
		$tabname = $this->TableName ($tabname);
		$sql = $this->_TableSQL($tabname,$lines,$pkey,$taboptions,$temporary); // crmv@fix
	...
	*/
	...
	{
		if (strlen($fsize) && !in_array($ty,array('X','B','T','D')) && strpos($ftype,'(') === false) {	//crmv@25671
			$ftype .= "(".$fsize;
	...
	// crmv@fix
	function getCreateTableString($tabname,$temporary = false){
	...
	// crmv@fix-e
	function _TableSQL($tabname,$lines,$pkey,$tableoptions,$temporary = false)
	{
		$sql = array();
		// crmv@fix
		if ($temporary)
			$tabname = $this->changeTableName($tabname);
		// crmv@fix-e
		...
		// crmv@fix
		$create = $this->getCreateTableString($tabname,$temporary);
		$s = "CREATE $create (\n";
		// crmv@fix-e
		$s .= implode(",\n", $lines);
		if (sizeof($pkey)>0) {
