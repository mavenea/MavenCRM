...
class ADODB_oci8 extends ADOConnection {
  ...
  where owner='%s' and table_name='%s' order by column_id"; // when there is a schema
	...
	var $NLS_DATE_FORMAT = 'RRRR-MM-DD HH24:MI:SS';  // crmv@fix To include time, use 'RRRR-MM-DD HH24:MI:SS'
	...
	// crmv@148761
	var $sanitizeWords = array('comment','session', 'uid');
	var $quoteExpr = '"%s"';
	// crmv@148761
...
*/
	...
	{
		if (empty($ts) && $ts !== 0) return null; //crmv@fix date null
