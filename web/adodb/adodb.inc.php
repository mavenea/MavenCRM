 ...
 */
 ...
 if (!defined('_ADODB_LAYER')) {
	...
	*/
		...
		define('ADODB_FETCH_DEFAULT',3); // crmv@fix
...
*/
	...
	{
		if (empty($ts) && $ts !== 0) return null; //crmv@fix date
		if ($isfld) return $ts;
	...
	*/
	...
	//crmv@fix field name
	function sanitize_field_array($fields){
		if (!empty($this->sanitizeWords)) {
			$ret_fields = array();
			foreach ($fields as $key=>$value){
				if (in_array($key,$this->sanitizeWords)) {
					$key = sprintf($this->quoteExpr, $key);
				}
				$ret_fields[$key] = $value;
			}
			$fields = $ret_fields;
		}
		return $fields;
	}
	function quote_field_name($fname){
		if ($this->quoteExpr) {
			$fname = sprintf($this->quoteExpr, $fname);
		}
		return $fname;
	}
   ...
   /**
	...
	//crmv@25671
	var $blobSize = 4000; 	/// any varchar/char field this size or greater is treated as a blob
	...
	*/
	...
	class ADORecordSet_array extends ADORecordSet
	{
		...
		global $ADODB_FETCH_MODE,$ADODB_COMPAT_FETCH;
			...
			parent::__construct($fakeid); // fake queryID // crmv@188937
		...
		function MoveNext() 
		{
			//crmv@53827
			global $ADODB_EXTENSION;
			if ($ADODB_EXTENSION) return adodb_movenext($this);
			//crmv@53827e
