...
//@nileio table prefix other than vte cannot be used with the installation because vte is hardcoded
//need to remove or add functions to replace the vte prefix in installation schema
...
if((!$viewAttachment) && (!$viewAttachment) && $action != $Ajx_module."Ajax" && $action != "chat" && $action != "HeadLines" && $action != 'massdelete'  &&  $action != "DashboardAjax" && $action != "ActivityAjax")
{
	...
	if((!$skipFooters) && $action != "about_us" && $action != "vtchat" && $action != "ChangePassword" && $action != "body" && $action != $module."Ajax" && $action!='Popup' && $action != 'ImportStep3' && $action != 'ActivityAjax' && $action != 'getListOfRecords')
	{
		...
		//@nileio changed copyright and you may remove the license if you want to as well, however I updated the licnes.
		echo "<td align=right width=\"33%\"><span style='color: #fe0000;'>&copy; 2020-" . date('Y') . "  <a href='{$enterprise_website[0]}' target='_blank'>{$enterprise_website[1]}</a></span><span style='color: #255a9b;'> | <a href=\"javascript:mypopup($license_par)\">" . $app_strings['LNK_READ_LICENSE'] . "</a></span></td></tr></table>";
