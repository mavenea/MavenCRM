...
/**
 * Provides API to work with CRM Module
 * @package vtlib
 */
class Vtecrm_ModuleBasic {
    /** ID of this instance */
    var $id = false;
    var $name = false;
    var $label = false;
    ...
    function initializeProperties() {
        ...
        foreach ($prefs as $pname => $value) {
            switch ($pname) {
                case 'vtenext_min_version'://crmv@207991
                    $this->minversion = $value;
                    break;
                case 'vtenext_max_version'://crmv@207991
    ...
    function __handleVteCoreSchemaChanges() {
        ...
        global $table_prefix;
        Vtecrm_Utils::AlterTable($table_prefix.'_tab', 'version C(10)');
    }
    ...
    function __create($tabidtouse=false) { //crmv@36557
        ...
        if(!Vtecrm_Utils::CheckTable($table_prefix.'_tab_info')) {
            ...
            //FOREIGN KEY fk_1_vte_tab_info(tabid) REFERENCES vte_tab(tabid) ON DELETE CASCADE ON UPDATE CASCADE)',
        }
        if($this->minversion) {
            $tabResult = $adb->pquery("SELECT 1 FROM ".$table_prefix."_tab_info WHERE tabid=? AND prefname='vtenext_min_version'", array($this->id));//crmv@207991
            if ($adb->num_rows($tabResult) > 0) {
                $adb->pquery("UPDATE ".$table_prefix."_tab_info SET prefvalue=? WHERE tabid=? AND prefname='vtenext_min_version'", array($this->minversion,$this->id));
            } else {
                $adb->pquery('INSERT INTO '.$table_prefix.'_tab_info(tabid, prefname, prefvalue) VALUES (?,?,?)', array($this->id, 'vtenext_min_version', $this->minversion));//crmv@207991
            }
        }
        if($this->maxversion) {
            $tabResult = $adb->pquery("SELECT 1 FROM ".$table_prefix."_tab_info WHERE tabid=? AND prefname='vtenext_max_version'", array($this->id));//crmv@207991
            if ($adb->num_rows($tabResult) > 0) {
                $adb->pquery("UPDATE ".$table_prefix."_tab_info SET prefvalue=? WHERE tabid=? AND prefname='vtenext_max_version'", array($this->maxversion,$this->id));//crmv@207991
            } else {
                $adb->pquery('INSERT INTO '.$table_prefix.'_tab_info(tabid, prefname, prefvalue) VALUES (?,?,?)', array($this->id, 'vtenext_max_version', $this->maxversion));//crmv@207991
            }
        }
        ...
        Vtecrm_Profile::initForModule($this);

        self::syncfile();

        if($this->isentitytype) {
            Vtecrm_Access::initSharing($this);
        }
     ...
     */
    function __delete() {
        Vtecrm_Module::fireEvent($this->name,
            Vtecrm_Module::EVENT_MODULE_PREUNINSTALL);
    ...
    function delete() {
        if($this->isentitytype) {
            Vtecrm_Access::deleteSharing($this);
            Vtecrm_Access::deleteTools($this);
            Vtecrm_Filter::deleteForModule($this);
            Vtecrm_Block::deleteForModule($this);
            Vtecrm_Panel::deleteForModule($this);	//crmv@146434
        }
        $this->__delete();
        Vtecrm_Profile::deleteForModule($this);
        Vtecrm_Link::deleteAll($this->id);
        Vtecrm_Menu::detachModule($this);
    ...
    function initTables($basetable=false, $basetableid=false) {
        ...
        Vtecrm_Utils::CreateTable($this->basetable,"$this->basetableid I(19) PRIMARY",true);
        Vtecrm_Utils::CreateTable($this->customtable,"$this->basetableid I(19) PRIMARY", true);
    }

    /**
     * Set entity identifier field for this module
     * @param Vtecrm_Field Instance of field to use
     */
    ...
    function setDefaultSharing($permission_text='Public_ReadWriteDelete',$editstatus=0) {
        Vtecrm_Access::setDefaultSharing($this, $permission_text, $editstatus);
    }
    // crmv@193317e
    ...
    function allowSharing() {
        Vtecrm_Access::allowSharing($this, true);
    }
    /**
     * Disallow module sharing control
     */
    function disallowSharing() {
        Vtecrm_Access::allowSharing($this, false);
    }
    ...
    function enableTools($tools) {
        ...
        foreach($tools as $tool) {
            Vtecrm_Access::updateTool($this, $tool, true);
        }
    }
    ...
    function disableTools($tools) {
        ...
        foreach($tools as $tool) {
            Vtecrm_Access::updateTool($this, $tool, false);
        }
    }

    /**
     * Add block to this module
     * @param Vtecrm_Block Instance of block to add
     */
     ...
     * @param Vtecrm_Panel Instance of panel to add
     */
     ...
     * @param Vtecrm_Filter Instance of filter to add
     */
     ...
     * @param Vtecrm_Block Instance of block to use to get fields, false to get all the block fields
     */
    function getFields($blockInstance=false) {
        $fields = false;
        if($blockInstance) $fields = Vtecrm_Field::getAllForBlock($blockInstance, $this);
        else $fields = Vtecrm_Field::getAllForModule($this);
        return $fields;
    }
