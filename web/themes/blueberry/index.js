...
//@nileio this is key to setup the theme correctly - it contains all functions sent to the browser and apply classes/etc.
jQuery(document).ready(function() {
	...
	//@nileio
	//TODO : consider removing this handler on ajaxcomplete.
	//what is the vtentitypreview do and where is that shown
...
var Theme = Theme || {
	...
	initilizeEvents: function() {
		...
		//@nileio this is where the select picker is initialised for filter and assigned to fields

		if (jQuery('#listView').length > 0 || jQuery('#KanbanViewContents').length > 0) {
			function initilizeListViewEvents() {

				//@nileio this is the Filter By drop down field - listed in the ListViewEntries.tpl
				setupSelectPicker('viewname', {
					//@nileio LiveSearch true is default , the default size is set to 6 elements in include/js/general.js to show the search box
					liveSearch: true,
					//@nileio changed to false - i need the dropdown to drop the menu to the left
					right: false,
				});
				//@nileio this is the Assigned To drop down field - listed in the ListViewEntries.tpl
				setupSelectPicker('lv_user_id', {
					//@nileio changed LiveSearch to false
					liveSearch: true,
					//@nileio changed to false
					right: false,
				});
			...
			//@nileio BUG: this call registers a callback for any ajax completed on the  document
			// https://api.jquery.com/ajaxcomplete/
			// this means that the initlisation of ListViewEvents happen every second which re-initilises the selectpicker for example
			// every second among other things caused by other ajax on the page such as the notifications, and calandar events, etc. which happen in the background
			// FIXIT : only run this if we need to using the function parameters

			//@nileio removed
			jQuery(document).ajaxComplete(function(e,xhr,settings) {
				//@nileio this is so bad
				//on every ajaxcomplete on the document we re-initalise the freaking dropdown lists !!!
				//this is insane

				initilizeListViewEvents();
				// crmv@173746
				//on every ajaxcomplete on the document we do this !!!
				//completely insance
