...
if (!file_exists('install/_install_request.inc.php') && isset($dbconfig['db_hostname']) && $dbconfig['db_status']!=='_DB_STAT_') {
    fwrite(fopen('logs/cronproclog.txt','a'),date('d-m-Y H:i:s'). ' - cron jobs starting.' . PHP_EOL);
   // exit('disable cron for now');
} else {
    fwrite(fopen('logs/cronproclog.txt','a'),date('d-m-Y H:i:s'). ' - application pending install.' . PHP_EOL);
    exit('application is not yet configured.');
}
// crmv@91979
